...
type Context = { linteCode?: string; taskId?: string };
...
async function insertLog(
  ...
  try {
    await sql`
      INSERT INTO automation_log (level, flow, linte_code, task_id, message)
      VALUES (${level}, ${flow}, ${context?.linteCode ?? null}, ${context?.taskId ?? null}, ${message})
    `;
  } catch (err) {
